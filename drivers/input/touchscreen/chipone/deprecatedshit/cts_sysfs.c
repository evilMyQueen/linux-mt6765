...
#define COMP_CAP_TEST_DATA_FILENAME     "comp-cap-test-data.txt"
    ...
    count += scnprintf(buf + count, PAGE_SIZE,
        "Factory test, fw version: 0x%04x, ELAPSED TIME: %lldms\n",
        fw_ver, ktime_to_ms(delta_time));
...
static int cts_sysfs_add_device(struct device *dev)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
    int ret = 0, i;
...
static void cts_sysfs_remove_device(struct device *dev)
{
    int i;
